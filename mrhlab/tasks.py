...
def style_transfer_task(contentfilename, stylefilename):
    ...
    styletransfer(stylefilename, contentfilename, UPLOAD_STYLE_PATH, UPLOAD_CONTENT_PATH, UPLOAD_STYLE_PATH)

def styletransfer(style_file_name, content_file_name, style_path, content_path, reslut_path, epochs=1, steps_per_epoch=50):
    @tf.function()
    ...
    style_image = load_img(os.path.join(style_path, style_file_name))
    content_image = load_img(os.path.join(content_path, content_file_name))
    ...
    plt.imsave(os.path.join(reslut_path, result_file_name), image.numpy()[0])
